...
def dynamicBU(N, input_list, count_list):

    for i in range(1,N):
        for j in range(1,N):
            count_list[i][j] = min(count_list[i-1][j], count_list[i][j-1]) + input_list[i][j]

def dynamicTD(N, input_list, count_list):
    ...
    if x == 0 and y == 0:
        count_list[x][y] = input_list[x][y]
    else:
        if x == 0:
            count_list[x][y] =dynamicTD((x,y-1),input_list, count_list) + input_list[x][y]
        
        elif y == 0:
            count_list[x][y] =dynamicTD((x-1 ,y), input_list, count_list) + input_list[x][y]
        
        else:
            count_list[x][y] = min(dynamicTD((x-1,y), input_list, count_list), dynamicTD((x,y-1), input_list, count_list)) + input_list[x][y]
...
def logic(input_file_path, output_file_path):
    ...
    #dynamicBU(N, input_list, count_list)
    dynamicTD((N-1,N-1), input_list, count_list)
